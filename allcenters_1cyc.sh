...
export ndays=${ndays:-${2:-8}}          ;#number of days back for verification
export fdays=${fdays:-${3:-5}}          ;#forecast length in days to be verified 

#---xCRON---------------------------------------------
export useXCRON=${useXCRON:-${4:-"NO"}}
if [[ ${useXCRON} = "YES" ]];then
  export pid=${pid:-${5:-5}}
  export xCronWorkDir=${xCronWorkDir:-${6:-"~/"}}
  export JobReportFile=${JobReportFile:-${7:-"~/xcronReport.txt"}}
  source ${xCronWorkDir}/${pid}.sh
fi
#-----------------------------------------------------

## -- verification parameters
export fcycle=${fcycle:-"00 12"}               ;#forecast cycles
export vhrlist=${vhrlist:-"00"}    ;#verification hours for each day
export vtype=${vtype:-anom}
#export vnamlist=${vnamlist:-"T"}
export mdlist=${mdlist:-"gfs"}
export levlist=${levlist:-"P850"}
export reglist=${reglist:-"G2"}
export caplist=${caplist:-"$mdlist"}                        

export webhost=${webhost:-""}
export webhostid=${webhostid:-"$LOGNAME"}
export vsdb_data=${vsdb_data:-/data-nas/xa38/vsdb_data}
export sorcdir=${sorcdir:-/home/xa38/GVER/map_util}
export ftpdir=${ftpdir:-""} 
export doftp=${doftp:-"NO"}                      ;#ftp maps to web site
#export mapdir=${mapdir:-/home/xa38/GVER/stmpd2/nwpvrfy/stat/map}                          
...
export rundir=${rundir:-/home/xa38/GVER/stmpd2/nwpvrfy/stats/map_plot}
...
export FFLAG=${FFLAG:-" "}
export GRADSBIN=${GRADSBIN:-/usr/local/grads-2.0.2/bin}
export imgconvert=${IMGCONVERT:-convert}                 

export vsdbhome=${vsdbhome:-/home/xa38/GVER}
export SUBJOB=${SUBJOB:-""}
export ACCOUNT=${ACCOUNT:-""}
export CUE2RUN=${CUE2RUN:-""}
export CUE2FTP=${CUE2FTP:-${CUE2RUN:-""}}
export GROUP=${GROUP:-""}
...
# -- search data for all models; write out binary data, create grads control file
  if [ $vnam = "WIND" ]; then
   ...
   echo $sorcdir/gen_scal.sh $vtype $vnam $reg $lev $edate $ndays "${fcycle}" $fdays $fhout $outname1 $maskmiss "$mdlist"
  fi
...
set -A mdnamec0  `echo $caplist |tr "[a-z]" "[A-Z]" `
...
*if (cintp = 0 & dist > 0.00004 ); cintp=10*substr(dist/40,1,6); endif
 if( dist > 0.004 ); cintp=substr(dist/4,1,4); endif
 if (cintp = 0 & dist > 0.0004 ); cintp=substr(dist/4,1,5); endif
 if (cintp = 0 & dist > 0.00004 ); cintp=substr(dist/4,1,6); endif
...
echo "$GRADSBIN/grads -bcp run cordieoff_${outname1}.gs"
...
echo "$GRADSBIN/grads -bcp run cordiff_${outname1}.gs"
...
echo "$GRADSBIN/grads -bcp run freq_${outname1}.gs"
...
EOF
  ...
  $SUBJOB -a $ACCOUNT -q $CUE2FTP -g $GROUP -p 1/1/S -t 0:30:00 -r 256/1 -j ftpcard -o ftpcard$$.out $rundir/ftpcard$$.sh
#--------------------------------------------
fi
#--------------------------------------------
...
if [[ ${useXCRON} = "YES" ]];then
  me=$(basename -- "$0")
  echo ${me}" "${pid}" done " >> ${xCronWorkDir}/${JobReportFile}
fi
#-----------------------------------------------------
