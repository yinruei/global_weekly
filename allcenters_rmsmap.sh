...
export ndays=${ndays:-${2:-2}}          ;#number of days back for verification
...
export vnamlist=${vnamlist}
export mdlist=${mdlist:-"gfs"}
export levlist=${levlist}
...
#export mapdir=${mapdir:-/home/xa38/GVER/stmpd2/nwpvrfy/stats/map}
...
export GRADSDIR=${GRADSDIR:-/usr/local/grads-2.0.2/data}
export GRADSCRP=${GRADSCRP:-/usr/local/grads-2.0.2/lib}
...
'quit'
EOF1
echo "$GRADSBIN/grads -bcp run ${var}p_${outname1}.gs"
...
'quit'
EOF1
echo "$GRADSBIN/grads -bcp run ${var}pmean_${outname1}.gs"
...
*--find maximum and minmum values
   ...
   i=i+1
   endwhile
   dist=cmax-cmin; cmin=cmin-1.7*dist
   if (cmax > 0 ); cmax=1.2*cmax ;endif
...
'quit'
EOF1
echo "$GRADSBIN/grads -bcp run ${var}_${outname1}${lev}.gs"
...
'quit'
EOF1
echo "$GRADSBIN/grads -bcp run ${var}dieoff_${outname1}${lev}.gs"
