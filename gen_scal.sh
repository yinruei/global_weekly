...
! and rms bias, write out in binary format for graphic display
       ...
       real*4             :: cor(fday,nday,nmd), rms(fday,nday,nmd)
       real*4             :: bias(fday,nday,nmd)   
       real*4             :: num(fday,nmd), mcor(fday,nmd)
       real*4             :: mrms(fday,nmd), mbias(fday,nmd)   
...
! read data
     ...
     +               sqrt((vsdb(4,i,j)-vsdb(1,i,j)**2)*(vsdb(5,i,j) 
     +               -vsdb(2,i,j)**2))
            if(cor(i,j,n).ge.-0.999 .and. cor(i,j,n).le.1.0) then
             rms(i,j,n)=sqrt(max(0.0d0,vsdb(4,i,j)+vsdb(5,i,j)
     +                       -2*vsdb(3,i,j)))
             bias(i,j,n)=vsdb(1,i,j)-vsdb(2,i,j)
             num(i,n)=num(i,n)+1
             mcor(i,n)=mcor(i,n)+cor(i,j,n)
             mrms(i,n)=mrms(i,n)+rms(i,j,n)
             mbias(i,n)=mbias(i,n)+bias(i,j,n)
             do k=1,bin
              if(cor(i,j,n).gt.binbnd(k).and.cor(i,j,n).le.binbnd(k+1)) 
     +           bincor(i,k,n)=bincor(i,k,n)+1.0
             enddo	
             do k=1,bin0
              if(cor(i,j,n).gt.binbnd0(k).and.cor(i,j,n)
     +           .le.binbnd0(k+1)) bincor0(i,k)=bincor0(i,k)+1.0
             enddo
            else
             cor(i,j,n)=bad
             rms(i,j,n)=bad
...
vars    4
cor     1 99  correlation
rms     1 99  rms errror 
bias    1 99  mean bias   
bincor  1 99  frequency distribution of AC
endvars
