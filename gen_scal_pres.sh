...
#------------------------------------------------------------
rm -f convert.f convert.x tmp.txt
...
! read data
     ...
     +                  sqrt(var_f(n,i,j,m)*var_a(n,i,j,m))
             mse(n,i,j,m)=max(0.0d0,(vsdb(4,n,i,j)+vsdb(5,n,i,j)
     +                    -2*vsdb(3,n,i,j)))
             ...
             do k=1,bin
              if(cor(n,i,j,m).gt.binbnd(k).and.cor(n,i,j,m)
     +           .le.binbnd(k+1)) bincor(n,i,k,m)=bincor(n,i,k,m)+1.0
             enddo
             do k=1,bin0
              if(cor(n,i,j,m).gt.binbnd0(k).and.cor(n,i,j,m)
     +           .le.binbnd0(k+1)) bincor0(n,i,k)=bincor0(n,i,k)+1.0
             enddo
