...
#-----------------------------------------------------------------------------
# VSDB Record of Vector Wind:
#    X1=MEAN[uf], X2=MEAN[vf], X3=MEAN[ua], X4=MEAN[va],
#    X5=MEAN[uf*ua+vf*va], X6=MEAN[uf*uf+vf*vf], X7=MEAN[ua*ua+va*va]
#    where  uf and vf are forecast winds,  and ua and va are analysis/observed winds,
#
...
#  Mean biases:    bias=sqrt[ X6 ] - sqrt[ X7 ]
#  MSE:            mse=( X6 + X7 - 2*X5 )
#  RMSE:           rmse=sqrt( X6 + X7 - 2*X5 )
#  MSE by mean difference: e_m={ [X1-X3]**2 + [X2-X4]**2 }
#  MSE by pattern variation
#    e_p={ var(V_f) + var(V_a) - 2*sqrt[var(V_F)*var(V_a)]*R }
#        =[ mse - e_m ]
...
export exedir=${exedir:-/home/xa38/GVER/stmpd2/nwpvrfy/stats/map_plot}
if [ ! -s $exedir ]; then mkdir -p $exedir; fi
cd $exedir

export vsdb_data=${vsdb_data:-/data-nas/xa38/vsdb_data}
export NWPROD=${NWPROD:-/lib/nwprod}
...
! write out in binary format for graphic display
       ...
       integer, parameter :: nmd=${nmdcyc}, ns=7
       integer, parameter :: bin0=20
       real*4             :: points(nlev,fday,nday)
       real*8             :: vsdb(ns,nlev,fday,nday)
       real*4             :: cor(nlev,fday,nday,nmd)
       real*4             :: rms(nlev,fday,nday,nmd)
       real*4             :: mse(nlev,fday,nday,nmd)
       real*4             :: bias(nlev,fday,nday,nmd)   
       real*4             :: e_m(nlev,fday,nday,nmd)
       real*4             :: e_p(nlev,fday,nday,nmd)
       real*4             :: msess(nlev,fday,nday,nmd)
       real*4             :: var_f(nlev,fday,nday,nmd)
       real*4             :: var_a(nlev,fday,nday,nmd)
       real*4             :: rvar(nlev,fday,nday,nmd)    
       real*4             :: num(nlev,fday,nmd), mcor(nlev,fday,nmd)
       real*4             :: mrms(nlev,fday,nmd), mmse(nlev,fday,nmd)
       real*4             :: mbias(nlev,fday,nmd)   
       real*4             :: me_m(nlev,fday,nmd),me_p(nlev,fday,nmd)
       real*4             :: mvar_f(nlev,fday,nmd)
       real*4             :: mvar_a(nlev,fday,nmd),mrvar(nlev,fday,nmd)
       real*4             :: mmsess(nlev,fday,nmd)
       real*4             :: bincor(nlev,fday,bin,nmd), binbnd(bin+1)
       real*4             :: bincor0(nlev,fday,bin0), binbnd0(bin0+1)
       real*4             :: fmiss(nlev,fday,nday)
       integer            :: plev(nlev)
       integer            :: nchar(nlev,fday,nday,nmd)
       integer            :: nhead(nlev,fday,nday,nmd)
       ...
       do n=1,nlev
         if(nhead(n,i,j,m).eq.0) then
           ...
           if(points(n,i,j).eq.0) then
             ...
             var_f(n,i,j,m)=max(0.0d0,vsdb(6,n,i,j)-
     +                          vsdb(1,n,i,j)**2-vsdb(2,n,i,j)**2)
             var_a(n,i,j,m)=max(0.0d0,vsdb(7,n,i,j)-
     +                          vsdb(3,n,i,j)**2-vsdb(4,n,i,j)**2)
             cor(n,i,j,m)=(vsdb(5,n,i,j)-vsdb(1,n,i,j)*vsdb(3,n,i,j)-
     +                     vsdb(2,n,i,j)*vsdb(4,n,i,j))/ 
     +                  sqrt(var_f(n,i,j,m)*var_a(n,i,j,m))
             mse(n,i,j,m)=(max(0.0d0,(vsdb(6,n,i,j)+vsdb(7,n,i,j)-
     +                         2*vsdb(5,n,i,j))))
             rms(n,i,j,m)=sqrt(mse(n,i,j,m))
             bias(n,i,j,m)=sqrt(vsdb(6,n,i,j))-sqrt(vsdb(7,n,i,j))
             e_m(n,i,j,m)=((vsdb(1,n,i,j)-vsdb(3,n,i,j))**2
     +                    +(vsdb(2,n,i,j)-vsdb(4,n,i,j))**2)
             ...
             do k=1,bin
              if(cor(n,i,j,m).gt.binbnd(k).and.cor(n,i,j,m).le.
     +           binbnd(k+1))  bincor(n,i,k,m)=bincor(n,i,k,m)+1.0
             enddo
             do k=1,bin0
              if(cor(n,i,j,m).gt.binbnd0(k).and.cor(n,i,j,m).le.
     +           binbnd0(k+1)) bincor0(n,i,k)=bincor0(n,i,k)+1.0
             enddo
...
! save mean scores as the nday+1 record in time
       ...
       do m=1,nmd
        do n=1,nlev
        write(13,123) $yyyymm,plev(n),trim(mdname(m)),
     +                "_cor",(mcor(n,i,m),i=1,fday) 
        enddo
        do n=1,nlev
        write(14,123) $yyyymm,plev(n),trim(mdname(m)),
     +                "_rms",(mrms(n,i,m),i=1,fday)  
        enddo
        do n=1,nlev
        write(15,123) $yyyymm,plev(n),trim(mdname(m)),
     +                "_bia",(mbias(n,i,m),i=1,fday) 
        enddo
       enddo
