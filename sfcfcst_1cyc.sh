...
export doftp=${doftp:-"NOt"}        ;#ftp maps to web site
...
# -- search data for all models; write out binary data, create grads control file
  ...
  echo $sorcdir/gen_sfc.sh $vtype $vnam $reg $lev $edate $ndays "${fcycle}" $fdays $fhout $outname1 $maskmiss "$mdlist"
...
EOF
 ...
 $SUBJOB -a $ACCOUNT -q $CUE2FTP -g $GROUP -p 1/1/S -t 1:00:00 -r 256/1 -j ftpcard -o ftpcard$$.out ${rundir}/ftpcard$$.sh
#--------------------------------------------
fi
#--------------------------------------------
